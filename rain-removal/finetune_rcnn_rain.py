...
Usage:
    python finetune_rcnn_rain.py [options]
...
Examples:
    ...
    # Custom settings
    python finetune_rcnn_rain.py --images ./images_pre_processed --annotations ./rain_annotations.json --iterations 5000
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune Faster R-CNN on Rain Dataset"
    )
    parser.add_argument(
        "--images",
        default="./images_pre_processed",
        help="Path to training images directory (default: ./images_pre_processed)"
    )
    parser.add_argument(
        "--annotations",
        default="./rain_annotations.json",
        help="Path to annotations JSON file (default: ./rain_annotations.json)"
    )
    parser.add_argument(
        "--base-checkpoint",
        default="../FasterRCNN/finetune_bdd.pth",
        help="Path to base checkpoint to start from (default: ../FasterRCNN/finetune_bdd.pth)"
    )
    parser.add_argument(
        "--output-dir",
        default="../FasterRCNN/rain_finetuned",
        help="Directory to save training outputs (default: ../FasterRCNN/rain_finetuned)"
    )
    parser.add_argument(
        "--output-model-name",
        default="finetune_rain.pth",
        help="Name for the final model file (default: finetune_rain.pth)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Evaluate every N iterations (default: 300)"
    )
...
DATASET_CONFIG = {
    ...
    "train_annotations": args.annotations,
    "val_images": None,
...
FINETUNED_MODEL_NAME = args.output_model_name

# =============================================================================
# FUNCTIONS
# =============================================================================

def create_train_val_split(annotations_path, images_dir, val_ratio=0.2, output_dir="./temp_split"):
...
def setup_training_config(training_config, num_classes, device='cuda'):
    ...
    cfg.SOLVER.STEPS = (int(training_config['max_iterations'] * 0.6),
...
def main():
    ...
    if DATASET_CONFIG['val_images'] is None or DATASET_CONFIG['val_annotations'] is None:
    ...
    # Setup configuration
    cfg = setup_training_config(TRAINING_CONFIG, len(train_classes), TRAINING_CONFIG['device'])
    ...
    if os.path.exists(final_checkpoint):
        ...
        print(f"\nTo use this model for evaluation:")
        print(f"  python evaluation_a_rain.py mca --model finetuned")
        print(f"  python evaluation_a_rain.py wavelet --model finetuned")
    else:
