...
## Quickstart: run inference on your own images

The original codebase targeted CUDA-only training with PyTorch 0.4. The repo now
ships a light-weight inference path that works on recent PyTorch releases,
including CPU-only Macs, so you can drop arbitrary images into `datasets/` and
obtain segmentation masks.

1. **Create the virtual environment (requires Python 3.10):**
   ```bash
   cd /Users/andyzhang/Digital-Image-Processing/CCNet
   bash scripts/setup_env.sh          # uses python3.10 by default
   source .venv-ccnet/bin/activate
   ```
   > Tip: install Python 3.10 via `brew install python@3.10` or `pyenv install 3.10.14`
   > if your system Python is 3.13+.

2. **Download pretrained weights (Cityscapes, R=1 by default):**
   ```bash
   python scripts/download_weights.py --model r1
   # Files are stored under weights/ccnet_cityscapes_r1.pth, r2 / r2_ohem also available
   ```

3. **Prepare your dataset:** place images anywhere under `datasets/` (for example
   `datasets/custom/images`). The new helper script accepts any nested folder,
   automatically handles resizing/mean subtraction, and writes predictions to the
...
4. **Run semantic segmentation:**
   ...
   python tools/run_inference.py \
     ...
     --weights weights/ccnet_cityscapes_r1.pth \
     --output-dir datasets/custom/outputs \
     --device auto \
     --save-overlay
   ```
   - `--device auto` picks `cuda`, `mps`, or `cpu`.
   - `--long-side` (default 1024) constrains the inference resolution for memory efficiency.
   - `--save-overlay` stores both the palette mask (`*_mask.png`) and an RGB overlay (`*_overlay.png`).

5. **Results** land in `datasets/custom/outputs` (or any directory you pass in).
   Every image gets a 19-class Cityscapes mask colored with the standard palette.
   You can post-process these masks to build dataset statistics or feed them into
   downstream tooling.

The inference stack relies on:

- `requirements.txt` (modern PyTorch, torchvision, OpenCV, Pillow, etc.).
- `scripts/setup_env.sh` (bootstraps `.venv-ccnet` with pinned deps).
- `scripts/download_weights.py` (fetches Google Drive checkpoints via `gdown`).
- `tools/run_inference.py` (batch inference with preprocessing/CPU+GPU support).

Feel free to adapt `tools/run_inference.py` for other pretrained checkpoints—the
script exposes CLI knobs for class count, recurrence depth, output folder, and
overlay opacity.
